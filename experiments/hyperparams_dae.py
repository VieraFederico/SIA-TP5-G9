...
con salt 0.1 por default, que es el nivel canónico del DAE; --salts permite probar otros
niveles manualmente. La métrica es el error de reconstrucción contra el patrón LIMPIO; NO
se usa "objetivo <=1px".
...
grid_hyperparams y el motor único study_engine.run_study. No duplica nada.
    ...
    python3 main.py study hyperparams-dae                              # grid completo × salt 0.1
...
from experiments.ae import DAE_ENCODER_WIDTHS
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Estudio de hiperparámetros DAE (grid opt×lr×init × salt 0.1).")
    ...
    configs = build_configs(opts, lrs, inits, widths=DAE_ENCODER_WIDTHS)

    slug = hyperparams_slug({"data": args.data, "arch": configs[0]["architecture"],
                             "salt": "-".join(str(s) for s in salts),
